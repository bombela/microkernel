/*
 *	Pilote FAT12 très simple pour le premier lecteur disquette.
 *
 ...
 *	20/11/2004
 *	14/02/2005
 *	04/10/2005 - Modification de la copie de certain secteur, afin d'éviter les erreurs :
 *		"use of cast expressions as lvalues is deprecated" et rendre le code plus propre.
 *	18/02/2006 - La calcul du nombre de cluster max donnais en réalité le nombre de sercteur max.
 *		   - Et une énorme erreur lors de la recherche d'entrée FAT...
 */
...
static char FindFileEntry(u16 *DirClust, TFat12_FileEntry **CurrentEntry,u8 *Buf,u16 *ClustInBuf,boolean FreeEntry)
{
	...
	OutDbg("FindFileEntry -> %s.",Root?"Répertoire racine":"Sous répertoire");
	...
	while (*DirClust < Fat12_ValidEoc)
	{
		...
		{
			OutDbg("Chargement du cluster %s%u (Précédent : %u).",(Root?"(racine) ":""),*DirClust,*ClustInBuf);
			if (Root)
			{
...
char Fat12_Init()
{
	...
	OutDbg("-- Pilote FAT12 très simple. DEBUG --");
...
char Fat12_ReadFile(TFat12_FileInfo *FileInfo,u32 Count,void *Buf)
{
	...
	NbStartCluster = Count / 512; // Nombre de cluster entier à copier.
